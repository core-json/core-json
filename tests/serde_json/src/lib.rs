...
mod tests {
  ...
  fn json_escape(str: &str) -> String {
    // Use `serde_json` to encode this `String` to inherit its escaping policy
    let res = Value::String(str.to_owned()).to_string();
    // Skip the opening quote
    let res = res.chars().skip(1);
    let mut res = res.collect::<String>();
  ...
  fn compare_strings(a: &str, b: &[u8]) -> bool {
    json_escape(a) == str::from_utf8(b).unwrap()
  }
  ...
  */
  ...
  fn check_null(encoding: &[u8], _value: &Value, path: &[PathElement]) {
    ...
    descend(null, path, |null: core_json::Value<_, _>| assert!(null.is_null().unwrap()));
  }
  ...
  fn check_number(encoding: &[u8], value: &Value, path: &[PathElement]) {
    ...
    descend(number, path, |number: core_json::Value<_, _>| {
      ...
      if expected.is_i64() {
        ...
        assert!((number.as_f64().unwrap() - expected.as_f64().unwrap()).abs() < 0.1f64);
      }
    });
  }
  ...
  fn check_object(encoding: &[u8], value: &Value, path: &mut Vec<PathElement>) {
    ...
    // Check the length of the object
    {
      ...
      descend(object, path, |object: core_json::Value<_, _>| {
        assert!(object.is_object().unwrap());
    ...
    for (field, value) in value {
      ...
      check_value(encoding, value, path);
      path.pop();
    }
  }
  ...
  fn check_value(encoding: &[u8], value: &Value, path: &mut Vec<PathElement>) {
    match value {
  ...
  fn fuzz() {
    for i in 0 .. 100 {
      ...
      let mut path = vec![];
      check_value(bytes, &value, &mut path);
    }
  }
}
