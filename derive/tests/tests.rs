// This is in a `mod` so the `struct`s can use `pub(super)`
mod test_mod {
  use core_json_traits::{JsonF64, JsonDeserialize, JsonSerialize};
  use core_json_derive::{JsonDeserialize, JsonSerialize};

  #[derive(Clone, Default, Debug, JsonDeserialize, JsonSerialize)]
  pub(crate) struct MyStruct<
    T: 'static + Default + core::fmt::Debug + JsonDeserialize + JsonSerialize,
  > {
    abc: u64,
    pub de: u8,
    ...
    pub(in self::super) float: JsonF64,
    #[skip]
    pub skipped: Option<u64>,
  }

  impl<T: 'static + Default + core::fmt::Debug + JsonDeserialize + JsonSerialize> PartialEq
    for MyStruct<T>
  where
    T: PartialEq,
  {
  ...
  #[derive(Clone, PartialEq, Default, Debug, JsonDeserialize, JsonSerialize)]
  pub struct WithoutT {
    #[key("xyza")]
    abc: i64,
    de: u8,
